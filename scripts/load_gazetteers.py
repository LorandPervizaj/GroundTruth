...
def _upsert_complex(session, entry: dict, neighborhood_map: dict[str, int]) -> None:
...
def _upsert_building(session, entry: dict, neighborhood_map: dict[str, int], complex_map: dict[str, int]) -> None:
...
def load() -> None:
    ...
    try:
        max_id = session.query(Neighborhood.id).order_by(Neighborhood.id.desc()).limit(1).scalar() or 0
        ...
        for entry in gazetteer._data.get("complexes", []):
            _upsert_complex(session, entry, neighborhood_map)
